...
class AplikacjaGUI(Frame, object):
    ...
    def Prowadzacy(self, master):
        ...
        #Tu jest metoda odpowiedzialna za pierwszy komentarz na początku gry, i na pewno dałoby się to zoptymalizować.
        #Ale szczerze mówiąc przysporzyło mi to wszystko więcej problemów niż myślałem.

    def komentarz(self,master):
        self.komentarz = Label(self.master, text="Zaczynajmy! A oto pierwsze pytanie:", bg='gold')
        self.komentarz.place(x=0, y=300, width=600, height=50)
    ...
    #tutaj realizowana jest pozostała część komentarzy jeżeli chodzi o udzielanie odpowiedzi
    #i jeszcze informacja o wygranej w messageboxie, również w ubogiej wersji, nie mniej działa
        ...
        for i in range(4):
            if i == ktora_odpowiedz:
                if czy_dobra == "prawidłowa":
                    ...
                    self.komentarz = Label(self.master, text = "test", bg = 'gold')
                    self.komentarz.place(x=0, y=300, width=600, height=50)
                    if nr_pytania == 0:
                        self.komentarz = Label(self.master, text="I to jest poprawna odpowiedź! Następne pytanie:", bg='gold')
                        self.komentarz.place(x=0, y=300, width=600, height=50)
                    elif nr_pytania == 1:
                        self.komentarz = Label(self.master, text="Prawidłowo. Oto kolejne pytanie:", bg='gold')
                        self.komentarz.place(x=0, y=300, width=600, height=50)
                    elif nr_pytania == 2:
                        self.komentarz = Label(self.master, text="Zgadza się. Następne pytanie:", bg='gold')
                        self.komentarz.place(x=0, y=300, width=600, height=50)
                    elif nr_pytania == 3:
                        self.komentarz = Label(self.master, text="Całkiem nieźle sobie radzisz! Przejdźmy do następnego pytania: ", bg='gold')
                        self.komentarz.place(x=0, y=300, width=600, height=50)
                    elif nr_pytania == 4:
                        self.komentarz = Label(self.master, text="I to jest poprawna odpowiedź! Następne pytanie:", bg='gold')
                        self.komentarz.place(x=0, y=300, width=600, height=50)
                    elif nr_pytania == 5:
                        self.komentarz = Label(self.master, text="Wygląda na to, że połowa juz za nami. A oto kolejne pytanie:", bg='gold')
                        self.komentarz.place(x=0, y=300, width=600, height=50)
                    elif nr_pytania == 6:
                        self.komentarz = Label(self.master, text="Zgadza się. Następne pytanie:", bg='gold')
                        self.komentarz.place(x=0, y=300, width=600, height=50)
                        ...
                        self.komentarz = Label(self.master, text="To pytanie nie należało do najłatwiejszych, gratuluję. Tak brzmi kolejne pytanie:", bg='gold')
                        self.komentarz.place(x=0, y=300, width=600, height=50)
                    elif nr_pytania == 10:
                        self.komentarz = Label(self.master, text="To wielka chwila, od miliona dzieli Cię ostatnie pytanie. Gotów? Brzmi ono tak:", bg='gold')
                        self.komentarz.place(x=0, y=300, width=600, height=50)
                    elif nr_pytania == 11:
                        self.komentarz = Label(self.master, text="Gratulacje! Właśnie wygrałeś milion polskich złotych!", bg='gold')
                        self.komentarz.place(x=0, y=300, width=600, height=50)
                    ...
                    if nr_pytania == 0:
                        messagebox.showinfo("Przegrałeś!", "Wracasz z pustymi rękoma! Spróbuj szczęścia następnym razem!")
                        self.master.destroy()
                    elif nr_pytania == 1:
                        messagebox.showinfo("Przegrałeś!","Twoja wygrana wynosi 500zł")
                        self.master.destroy()
                    elif nr_pytania == 2:
                        messagebox.showinfo("Przegrałeś!","Twoja wygrana wynosi 1000zł")
                        self.master.destroy()
                    elif nr_pytania == 3:
                        messagebox.showinfo("Przegrałeś!","Twoja wygrana wynosi 2000zł")
                        self.master.destroy()
                    elif nr_pytania == 4:
                        messagebox.showinfo("Przegrałeś!","Twoja wygrana wynosi 5000zł")
                        self.master.destroy()
                    elif nr_pytania == 5:
                        messagebox.showinfo("Przegrałeś!","Twoja wygrana wynosi 10000zł")
                        self.master.destroy()
                    elif nr_pytania == 6:
                        messagebox.showinfo("Przegrałeś!","Twoja wygrana wynosi 20000zł")
                        self.master.destroy()
                    elif nr_pytania == 7:
                        messagebox.showinfo("Przegrałeś!","Twoja wygrana wynosi 40000zł")
                        self.master.destroy()
                    elif nr_pytania == 8:
                        messagebox.showinfo("Przegrałeś!","Twoja wygrana wynosi 75000zł")
                        self.master.destroy()
                    elif nr_pytania == 9:
                        messagebox.showinfo("Przegrałeś!","Twoja wygrana wynosi 125000zł")
                        self.master.destroy()
                    elif nr_pytania == 10:
                        messagebox.showinfo("Przegrałeś!","Twoja wygrana wynosi 250000zł")
                        self.master.destroy()
                    elif nr_pytania == 11:
                        messagebox.showinfo("Przegrałeś!","Twoja wygrana wynosi 500000zł")
                        self.master.destroy()
            else:
    ...
    def info_dobra_odpowiedz(self, nr_pytania):
        if nr_pytania == 11:
            messagebox.showinfo("GRATULACJE", "WYGRAŁEŚ MILION POLSKICH ZŁOTYCH!")
            self.master.destroy()
    ...
    #def przegrana(self):
        #if nr_pytania == 1:
           # messagebox.showinfo("Przegrałeś!","Udało Ci się zarobić:")
            #self.master.destroy()
